...
namespace DEShaders.Containers
{
    ...
    public enum DE_Publisher { DE_Environment, AlexanderElert }
...
namespace DEShaders.Utils
{
    ...
    public static class DE_VertexColor
    {
        ...
        public static void Recalculate(GameObject[] obj, SubmeshOption[] submeshOption, Vector3 colorModifier)
        {
            if (obj.Length != 1)
                return;
            if (obj[0] != null)
                        ...
                        for (int SubMeshIndex = 0; SubMeshIndex < SubMeshCount; SubMeshIndex++)
                        {
                            ...
                            if (submeshOption[c] == SubmeshOption.Cutout)
                            {
                                ...
                                Parallel.For(0, connections.Length, j =>
                                {
                                    ...
                                    {
                                        float dist = Vector3.Distance(Vector3.Lerp(tempMeshSet[0].vertices, tempMeshSet[1].vertices, 0.5f), tempMeshSet[i].vertices);
                            ...
                            if (submeshOption[c] == SubmeshOption.Billboard)
                            {
                                Parallel.For(0, SubMesh.vertices.Length, i =>
                                {
                                    ...
                                    colors[i] = new Color
                                    {
                                        r = Mathf.Lerp(0, 1, MeshVertices[i].y / distancePosMax.y) * colorModifier.x,
                                        g = 0,
                                        b = 0,
                                        a = alpha
                                    };
    ...
    {
        public static void Recalculate(GameObject obj, int samples = 512, float minRange = 0.0000000001f, float maxRange = 1.5f, float intensity = 5.0f)
        {
            ...
            foreach (MeshFilter mf in mfs)
            {
                ...
                for (int i = 0; i < l; i++)
                {
                    ...
                    colors[i].a = Mathf.Clamp01(1 - ((occlusion * intensity) / samples));
                    occlusion = 0;
                    if (i % 500 == 0)
                    {
                        EditorUtility.DisplayProgressBar(
