...
public class AE_Manager : EditorWindow
{
    ...
    public static void CreateAEManagerPath()
    {
        ...
        {
            var obj = CreateInstance(typeof(DE_PathManager));
            AssetDatabase.CreateAsset(obj, path);
            AssetDatabase.Refresh();
            EditorGUIUtility.PingObject(obj);
            var pm = (DE_PathManager)AssetDatabase.LoadAssetAtPath(path, typeof(DE_PathManager));
            pm.publisher = DE_Publisher.AlexanderElert;
        }
        else
        {
    ...
    public void OnGUI()
    {
        ...
        if (mainGUI == null)
        {
            mainGUI = new DEShadersGUI.MainGUI(
                ...
                actions: new System.Action[] { InfoGUI, SetupGUI, aboutGUI.OnGUI },
                name: "Æ Manager",
                version: version,
        ...
        if (e.control && e.shift && e.keyCode == KeyCode.I)
            Selection.activeObject = (DE_PathManager)AssetDatabase.LoadAssetAtPath(path, typeof(DE_PathManager));
    }
    void InfoGUI()
    {
        ...
        EditorGUILayout.LabelField("Please load first the package you'd like to install\nand then you can load the demo scenes (optional).\n\n" +
            "Current Demo Available Versions:\n" +
            "- Standard Render Pipeline\n" +
            "- URP (7.4.3 / 7.5.1 / 8.2.0)\n" +
            "- HDRP (7.4.3 / 7.5.1 / 8.2.0)\n", LabelStyle, GUILayout.Height(100));
    ...
    void SetupGUI()
    {
        ...
        if (!paths)
        {
            paths = (DE_PathManager)AssetDatabase.LoadAssetAtPath(path, typeof(DE_PathManager));
        }
        if (!paths)
        {
            EditorGUILayout.LabelField("AE Path.asset does not exist!", EditorStyles.boldLabel);
